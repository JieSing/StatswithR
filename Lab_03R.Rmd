...
```
FILL ME IN WITH WORDS
Max values of u, g, z, y are all 99.00, which does not agree (too large) with the other statistical variables
```
...
A cool thing about `dplyr` is that you can use a piping operator (%&gt;%) to 
have the output of one function be the input to the next. 
And you don't have to have only `dplyr` functions within the flow; for instance, you could pipe the first two rows your data frame to head:
```{r}
...
```{r}
# FILL ME IN
predictors %>% filter(.,i<25 & g>22) %>% head()
```
...
```{r}
# FILL ME IN
row = which(predictors$u!=99 & predictors$g!=99 & predictors$z!=99 & predictors$y!=99)
resp.new = response[row]

#predictors %>% filter(.,u!=99 & g!=99 & z!=99 & y!=99) ->resp.new 
#typeof(row)
...
```{r}
# FILL ME IN
pred.new %>% select(.,type) %>% group_by(.,type) %>% summarize(.,Number=n())
```
...
pred.newer %>% group_by(.,type) %>%
summarize(.,grMean=mean(gr),riMean=mean(ri))
...
```
FILL ME IN WITH WORDS
p-value is smaller than 0.05. We reject the null hypothesis that gr.faint and gr.bright are equal to each other.
```
