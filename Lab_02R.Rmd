...
x=seq(-5,5,by=0.01)
fx = dnorm(x)
plot(x,fx,typ='l',col='blue',xlab='x',ylab='f(x)')

abline(v=(smean-mu_o)/(sd/sqrt(ssize)),col='red') #test statistics
abline(v=qnorm(0.05),col='black') #boundary of rejection region
```
...
Z = (smean-mu_o)/(sd/sqrt(ssize))
pnorm(Z)

#pnorm is smaller than alpha. Reject the null hypothesis.
```
...
fx = dnorm(x)
plot(x,fx,typ='l',col='blue',xlab='x',ylab='f(x)')
abline(v=(smean-mu_o)/(sd/sqrt(ssize2)),col='red') 
Z = (smean-mu_o)/(sd/sqrt(ssize2))
pnorm(Z)

#pnorm is larger than alpha. Accept the null hypothesis.
```
...
x=seq(-5,5,by=0.01)
fx = dnorm(x)
plot(fx,typ='l',col='blue',xlab='x',ylab='f(x)')
...
The frequency for different bins of p value can be approximated to be the same. Yes.
...
```{r}
# FILL ME IN
t.test(x,mu=10.5,alternative='less')
```
