...
```
FILL ME IN
Row 1 to row 2 = \sqrt(3)
Row 2 to row 3 = \sqrt(3)
```
...
```{r}
# FILL ME IN
dist(df, method = "euclidean", diag = FALSE, upper = FALSE, p = 2)
```
...
df %>% filter(.,dec>56 & ra<264 & velocity.los< -250 & velocity.los> -350) -> df
df %>%
  mutate(.,gr=mag.g-mag.r,ri=mag.r-mag.i) -> df.new
df.new %>% 
  dplyr::select(.,-c(mag.g,mag.r,mag.i,velocity.los)) -> df.new
...
```
FILL ME IN
K-means clustering with 10 clusters of sizes 34, 10, 116, 71, 362, 178, 24, 145, 98, 180
...
```{r}
df <- read.csv("C:/Users/Jie Sing Yoo/Downloads/CMU Coursework/Overview of Statistical Learning and Modeling/R_labs/data/diamonds.csv",stringsAsFactors=TRUE)
...
set.seed(202)
# FILL ME IN
km.out = kmeans(scale(df.small),3)
ggpairs(df.small,mapping=aes(color=factor(km.out$cluster)))

#there are natural clusters on all pairs of variables except depth vs table
```
