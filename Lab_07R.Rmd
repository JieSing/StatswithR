...
# FILL ME IN
cor(df) %>%
corrplot(.,method="color",tl.pos="n")
```
...
```
FILL ME IN
take the pca more than 95%, which is 6
```
...
```{r}
# FILL ME IN
pcax = data.frame(pca$x[,1:2])
ggplot(data=pcax,mapping=aes(x=PC1,y=PC2,color=response)) + geom_point(size=0.5)
pcax[0:5,]
#pca$x[,1:2]
```
...
```{r}
# FILL ME IN
pcax2 = data.frame(pca$x[,2:3])
ggplot(data=pcax2,mapping=aes(x=PC2,y=PC3,color=response)) + geom_point(size=0.5)
```
...
```{r}
# FILL ME IN
ggplot(data=pcax,mapping=aes(x=PC1,y=response)) + geom_boxplot(color='black',fill='yellow')
