...
{
return( sum((data$y-(par[1]*((data$x)^3)+par[2]*((data$x)^2)+par[3]*(data$x)+par[4]))^2/(data$e)^2) )  # chi-square, coded
}
par <- c(1,1,1,1) # initial guess for alpha
...
```
FILL ME IN
Consider a 95% confidence interval. 0.22 > 0.05, so we accept the null hypothesis.
```
...
```{r}
# FILL ME IN
loess.out = loess(y~x,data=df,weights=1/(df$e)^2,span=0.8)
ggplot(data=df,mapping=aes(x=x,y=y)) +
  geom_errorbar(aes(ymin=y-e, ymax=y+e), width=.1,color="blue") +
  geom_point(color="firebrick") +
  geom_line(mapping=aes(x=loess.out$x,y=loess.out$fitted),color="olivedrab") + labs(title='loess with span = 0.8')
```
```
FILL ME IN
0.8
...
ggplot(data=df,mapping=aes(x=x,y=y)) +
  geom_errorbar(aes(ymin=y-e, ymax=y+e), width=.1,color="blue") +
  geom_point(color="firebrick") +
  geom_line(mapping=aes(x=loess.out$x,y=loess.out$fitted),color="olivedrab") + geom_line(mapping=aes(x=loess.out$x,y=loess.out$fitted+p$se),color="coral",linetype="dashed") + geom_line(mapping=aes(x=loess.out$x,y=loess.out$fitted-p$se),color="black",linetype="dashed")
```
