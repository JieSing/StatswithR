...
out.bgB = bestglm(df.train,family=gaussian,IC='BIC')
out.bgB$BestModel

out.bgA = bestglm(df.train,family=gaussian,IC='AIC')
out.bgA$BestModel
```
```
FILL ME IN
...
```{r}
# FILL ME IN
cost.predA = predict(out.bgA$BestModel,newdata=df.test)
mean((obs-cost.predA)^2)

cost.predB = predict(out.bgB$BestModel,newdata=df.test)
mean((obs-cost.predB)^2)
...
out.bg.bic = bestglm(df.train,IC='BIC')
