...
```{r}
# FILL ME IN
df = read.csv("C:/Users/Jie Sing Yoo/Downloads/CMU Coursework/Overview of Statistical Learning and Modeling/R_labs/data/heart_disease.csv",header=TRUE)
...
```{r}
# FILL ME IN
df_new <- subset(df,select=-c(Gender,Drugs,Complications)) %>% gather(.)
...
df_bar<- df %>% dplyr::select(.,Gender, Drugs,Complications) %>% gather(.)
  
```

```{r}
ggplot(data=df_bar,mapping=aes(x=value)) + geom_bar(color="red",fill="blue") + facet_wrap(~key,scales='free_x') 
...
```{r}
# FILL ME IN
plot1 = ggplot(data=df,mapping=aes(x=Gender,y=Cost)) + geom_boxplot(color="blue",fill="yellow") 
plot2 = ggplot(data=df,mapping=aes(x=Drugs,y=Cost)) + geom_boxplot(color="blue",fill="yellow") 
plot3 = ggplot(data=df,mapping=aes(x=Complications,y=Cost)) + geom_boxplot(color="blue",fill="yellow") 
...
```
```{r}
ggplot(data=df,mapping=aes(x=Age, y=Comorbidities)) +
geom_point(color="blue",size=0.5) 

```
```{r}
ggplot(data=df,mapping=aes(x=Age, y=Duration)) +
geom_point(color="blue",size=0.5) 
```
...
df$Complications = as.integer(df$Complications)
df$Drugs = as.integer(df$Drugs)

df %>%
dplyr::select(.,Cost,Age,Interventions,Drugs,ERVisit,Complications,Comorbidities,Duration) %>%
cor(.) %>%
...
df %>%
ggpairs(.,columns=2:9,upper=list(continuous='points'))
```
