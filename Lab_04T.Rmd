...
# FILL ME IN
ggplot(data=df,mapping=aes(x=redshift)) +
  geom_histogram(bins=60,fill="black")
```
...
ggplot(data=df,mapping=aes(x=redshift)) +
  ...
  labs(y='Number of Galaxies at Redshift z', x = 'Redshift z')
```
...
```{r}
# FILL ME IN
new <- df %>% dplyr::group_by(type) %>% summarise(.,count=n())
  
ggplot(data=df,mapping=aes(x=type)) + geom_bar(color="red",fill="blue") + geom_text(mapping =aes(x=type,y=count,label=count), data = new, stat = "identity")
ggplot(data=new,mapping=aes(x=type,y=count/sum(count))) + geom_bar(stat='identity',color="red",fill="blue")  + labs(y='probability mass function')
```
...
geom_histogram(color="blue",bins=25,alpha=0.2,position='Identity') +
scale_x_continuous(c(20,32))
```
...
```{r}
# FILL ME IN
df_new <- df %>% dplyr::select(.,u, g, r, i, z, y) %>% gather(.)

ggplot(data=df_new,mapping=aes(x=value)) +
geom_histogram(color="blue",fill="yellow",bins=25,position='Identity') +
...
# FILL ME IN
df %>%
  mutate(.,ug=u-g,gr=g-r,ri=r-i,iz=i-z,zy=z-y) -> df.newer 
