...
```{r}
# FILL ME IN
xtest = rnorm(seq(0,9.9,by=0.1),5,2)
sum(xtest<5)/length(xtest)
...
```{r}
# FILL ME IN
xtest = seq(-5,5,by=0.1)
ytest=dnorm(xtest)
plot(xtest,ytest,xlab='x',ylab='f_x(x)')
...
plot(xtest,ytest,xlab='x',ylab='f_x(x)',typ="l",col='blue')
z=dt(xtest,3)
lines(xtest,z,col='red')
```
...
xtest = seq(-5,25,by=1)
y1 = dbinom(xtest,size=20,prob=0.5)
y2 = dbinom(xtest,size=20,prob=0.1)
y3 = dbinom(xtest,size=20,prob=0.9)
plot(xtest,y1,col='red',ylim=c(0,0.3),typ='l')
lines(xtest,y2,col='black')
lines(xtest,y3,col='blue')
```
